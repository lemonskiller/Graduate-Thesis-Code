...
%% 路径设置
% 运行位置为 ph2文件夹
addpath '../../matlab_addpath'

%% 导入数据
disease_name = 'ADNI';
rawdata=importdata('../adni_p0.05_del_nagene.csv');
outname = '../fig2_3_result/ADNI_delnagene_L1_L21_dst---10.txt';
train_num =floor((size(rawdata.data,2)-1)*0.8);
% 原始数据
...
% 训练集
rand_idx=randperm(size(sample,1));
rand_train_idx=rand_idx(1:train_num);
train_label = label(rand_train_idx);
train_sample = sample(rand_train_idx,:);
% 测试集 
rand_test_idx=rand_idx(1+train_num:end);
test_label = label(rand_test_idx);
test_sample = sample(rand_test_idx,:);
...
for dst=0.2 %step:step:1
    for L1=0.2 %step:step:1
        for L21=0.6 %step:step:1
