# -*- coding:utf-8 -*-
'''
    训练集随机取百分比60,80,100，重复10次，测试集不变
'''
...
for percenti in percent:
    ...
    for repeati in range(1,1+total_repeat):
        ...
        model = linear_model.ElasticNetCV(alphas=[0.1], l1_ratio=[.2]).fit(X, Y)
        model.fit(X, Y)
    ...
    # res.to_csv(r'../fig2_6_result/net_3geo_train_repeat_percent'+str(percenti)+'_top'+str(featnum)+'.csv')
