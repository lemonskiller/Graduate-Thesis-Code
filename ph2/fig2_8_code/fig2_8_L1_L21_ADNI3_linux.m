...
% rawdata=importdata('C:\lemon\py_test\feature_selection\adni_p0.05_del_nagene.csv');

outname = '../fig2_8_result/3322adni_L1_L216.txt';
% train_num = 502+499;
...
%% 5-fold CV
% 训练集
train_label = label(1:end-201);
train_sample = sample(1:end-201,:);
% 测试集-打酱油
test_label = label(end-200:end);
test_sample = sample(end-200:end,:);
...
for dst=0
    for L1=0.2%step:step:1
        for L21=0.2%step:step:1
            ...
            scale_method = 2; %svmscale输入要求:（1）x:样本数*特征数的矩阵（2）method：1缩放到[-1,1];0:缩放到[0,1]
