...
for with_withouti in with_without:
    ...
    for i in range(total_repeat):
        ...
        if with_withouti=='without':
            model = AdaBoostClassifier(DecisionTreeClassifier(max_depth=3),n_estimators=70,random_state=12)
        else:
            model = AdaBoostClassifier(DecisionTreeClassifier(max_depth=3),n_estimators=30,random_state=12)
