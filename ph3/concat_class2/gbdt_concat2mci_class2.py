...
for with_withouti in with_without:
    ...
    for i in range(total_repeat):
        ...
        if with_withouti=='without':
            model = GradientBoostingClassifier(n_estimators=30,random_state=12) #with 80;without 300
        else:
            model = GradientBoostingClassifier(n_estimators=80,random_state=12) #with 80;without 300
