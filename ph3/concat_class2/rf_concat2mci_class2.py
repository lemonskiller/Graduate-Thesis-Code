...
for with_withouti in with_without:
    ...
    for i in range(total_repeat):
        ...
        if with_withouti=='without':
            model = RandomForestRegressor(n_estimators=1,random_state=12) # with 500
        else:
            model = RandomForestRegressor(n_estimators=5,random_state=12) # with 500
    ...
    # pd.DataFrame(cr).to_csv('./concat2mci/rf_'+with_withouti+'_meth.csv')
    ...
    # fig.savefig('./concat2mci/rf_'+with_withouti+'_meth.svg', dpi=600, bbox_inches='tight')
