...
for with_withouti in with_without:
    ...
    for i in range(total_repeat):
        ...
        if with_withouti=='without':
            model = XGBClassifier(reg_alpha=.1,reg_lambda=.4,n_estimators=100,random_state=2020)
        else:
            model = XGBClassifier(reg_alpha=.2,reg_lambda=.1,n_estimators=30,random_state=2020)  # without 1 1 50 81;2 5 50 75.9
