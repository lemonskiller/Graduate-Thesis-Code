...
for with_withouti in with_without:
    ...
    for i in range(total_repeat):
        ...
        K_exp  = Kinterface(data=train_sample_feati, kernel=rbf_kernel,kernel_args={"sigma":35}) # RBF kernel 

            # K_exp  = Kinterface(data=train_sample_feati, kernel=poly_kernel,kernel_args={"degree":1,"b":40}) # RBF kernel ,"gamma":1
    ...
    # # Binarize the output
    # y_test = label_binarize(test_label, classes=[1, 2])
