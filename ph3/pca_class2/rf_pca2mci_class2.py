...
for with_withouti in with_without:
    ...
    for i in range(total_repeat):
        ...
        if with_withouti=='with':
            model = RandomForestRegressor(n_estimators=7,random_state=12)
        else:
    ...
    # pd.DataFrame(cr).to_csv('./pca_add2mci/rf_'+with_withouti+'_meth.csv')
    ...
    # fig.savefig('./pca_add2mci/rf_'+with_withouti+'_meth.svg', dpi=600, bbox_inches='tight')
