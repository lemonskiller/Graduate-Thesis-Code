...
for with_withouti in with_without:
    ...
    for i in range(total_repeat):
        ...
        if with_withouti=='without':
            model = Ridge(alpha=.1,random_state=2020) # with 10-43
        else:
            model = Ridge(alpha=10,random_state=2020)
    ...
    # test
    test_sample_feat = MinMaxScaler().fit_transform(test_sample_feat)
    # test_sample_feat =best_scaler.transform(test_sample_feat)
