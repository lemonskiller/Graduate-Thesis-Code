...
for with_withouti in with_without:
    ...
    for i in range(total_repeat):
        ...
        if with_withouti == 'without':
            model = AdaBoostClassifier(DecisionTreeClassifier(max_depth=6), n_estimators=100, random_state=12)
        else:
            model = AdaBoostClassifier(DecisionTreeClassifier(max_depth=6), n_estimators=100, random_state=12)
    ...
    # pd.DataFrame(cr).to_csv('./pca_add/'+algo+'_'+with_withouti+'_meth.csv')
    ...
    h = sns.heatmap(conf_mat, annot=True, xticklabels=['NC', 'MCI', 'AD'], yticklabels=['NC', 'MCI', 'AD'], fmt='g',
                    cmap=cmapi, annot_kws={'size': 26, 'weight': 'bold', 'family': 'Times New Roman'}, linewidths=1,
                    cbar=True, square=True)  # 画热力图
    ...
    # fig.savefig('./pca_add/'+algo+'_heatmap_'+with_withouti+'_meth.svg', dpi=600, bbox_inches='tight')
