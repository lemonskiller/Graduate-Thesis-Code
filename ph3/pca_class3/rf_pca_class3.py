...
for with_withouti in with_without:
    ...
    for i in range(total_repeat):
        ...
        if with_withouti=='without':
            model = RandomForestRegressor(n_estimators=6,random_state=2020)
        else:
            model = RandomForestRegressor(n_estimators=6,random_state=2020)
    ...
    # pd.DataFrame(cr).to_csv('./pca_add/rf_'+with_withouti+'_meth.csv')
    ...
    # fig.savefig('./pca_add/rf_'+with_withouti+'_meth.svg', dpi=600, bbox_inches='tight')
