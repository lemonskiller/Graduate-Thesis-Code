...
def create_graph(scores, feature_pheno, num_nodes):
    ...
    for l in scores:
        ...
        if l in ['AGE', 'PTEDUCAT']:  # 连续数值变量，定量的表型分数
        ...
        else:  # 离散数值变量，即定性，比如性别、APOE4、认知评分
            for k in range(num_nodes):
                for j in range(k + 1, num_nodes):
...
def my_train_test_split(label, fold, test_per_class, val_per_class):
    ...
    for i in range(fold):
        ...
        train_ind0=random.sample(list(remain_train_ind0),train_per_class)
...
def train_fold(algo, with_withouti,train_ind, test_ind, val_ind, graph_feat, weight_graph_feat, features, features_mri,features_meth,features_grade, y, y_data, params):
    ...
    if params['priorInfo']:
        ...
        # final_graph = np.eye(x_data.shape[0])
...
def main():
    ...
    parser.add_argument('--hidden', default=25, type=int, help='Number of filters in hidden layers (default: 16)')
    ...
    parser.add_argument('--adaptive', default=False, type=bool, help='whether to use adaptive adj')
    parser.add_argument('--priorInfo', default = False, type = bool, help = 'whether to use prior information')
    ...
    labels_onehot = label_binarize(labels, np.arange(num_classes))
    ...
    if args.folds == 5:  # run cross validation on all folds
        ...
        scores = Parallel(n_jobs=1)(delayed(train_fold)(algo, with_withouti,train_ind, test_ind, test_ind, graph, weight_graph, features, features_mri,features_meth,features_grade,
                                     ...
                                     list(my_train_test_split(labels,5,70,10)))
